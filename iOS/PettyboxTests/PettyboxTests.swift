import XCTest
@testable import Pettybox

@MainActor
final class PettyboxTests: XCTestCase {
    var store: PettyboxStore!
    ...
    func testFreeLimitBlocksAddition() {
        store.isPro = false
        for i in 0..<(PettyboxStore.freeLimit + 5) {
            store.add(title: "Filler \(i)", amount: 1.0)
        }
        XCTAssertEqual(store.entries.count, PettyboxStore.freeLimit)
    }
    ...
    func testCanAddMoreWhenPro() {
        ...
        XCTAssertGreaterThan(store.entries.count, PettyboxStore.freeLimit)
    }
    ...
    func testFreeLimitAboveSeedCount() {
        XCTAssertGreaterThan(PettyboxStore.freeLimit, 3)
    }
}
